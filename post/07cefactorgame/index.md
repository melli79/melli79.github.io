...
Then we ask for the board size, i.e. the upper limit.  For a reasonable game, we limit to 5..100 numbers.
...
The loop will run indefinitely, therefore `for (;;) { ... }`.  In the beginning, we will show the remaining numbers.  Therefore, we have inserted all numbers into the `set<int> board` and now loop over this (odered) set.

If this is the first move (and the `lastNumber` is 0), then we will remind the player to pick an even number.  We will then loop over picking such a number, but we need to work out the condition.  The first part is obvious, i.e. the number must still be on the board, but we need more conditions.  The first condition is already sufficient to repeat the loop, so all following conditions must be connected with `||`.  If `lastNumber==0`, then we also need to test whether the choice is even.  So we add `lastNumber==0&&choice%2!=0`, because we have to repeat if the choice is not even.  If the `lastNumber>0`, then we need to test whether `choice` is not a multiple of `lastNumber` or `choice==0` or `lastNumber` is not a multiple of `choice`.  In either case, we have to loop.  Also the third case (`lastNumber>0`) is an alternative, so we need to connect with `||`.  In total, we modify the `while (...)`-condition as follows:
...
It is now possible to play the game.  Please give it a try.  Do you notice any problem?

## 1.2 Testing for Game Over
...
Well, if the `lastNumber` is not 0, then we should check whether there is either a multiple or a divisor left.  We can do this as follows:
  ...
  int main() {
    ...
    for (;;) {
      ...
      if (lastNumber==0) {
        ...
        for (int m=2*lastNumber; m<=limit; m+=lastNumber) {
          if (board.find(m)!=board.end()) {
            hasOption = true;
            break;
          }
        }
        if (!hasOption)  for (int d : computeDivisors(lastNumber)) {
...
You should carefully check the play again in order to verify that the game indeed works well.

## 1.3 Cleaning up a bit

We have a running program, but remember that the goal of writing a computer program consists not only of writing a compiling and running program.  Firstly, we want to make sure we and hopefully also others understand our programs.  Secondly, we wish to make obvious that the program works correctly.
...
Why, because if we choose a reasonable name for the function such as `computeDivisors`, then it already says that it will compute the divisors.  Also, we don't have to write “divisors of a number”, because the argument is `int n`, so the number is an integer (and we would rightfully assume that it is used).
...
```C++
  ...
  bool hasOption(int lastNumber, const set<int>& board) {
    if (board.empty())
      return false;
    const int limit = *board.rbegin();
    for (int m=2*lastNumber; m<=limit; m+=lastNumber) {
      if (board.find(m)!=board.end()) {
        return true;
      }
    }
    for (int d : computeDivisors(lastNumber)) {
      if (board.find(d)!=board.end()) {
        return true;
      }
    }
...
Note that this way, we have been able to get rid of the variable `bool hasOption`, because as soon as we have found an option, we simply `return true`.  The default value `hasOption = false` is returned as a last resort (at the end of the function) if we haven't found any option by then.
...
If you would start the game now, then you see that every second move nothing happens.  That is because we have not yet implemented the AI.  We will do that next.  But first, let us see how it fits in:
...
0. if there is noting left on the board, then we cannot choose and lose.
1. We run through the whole board and consider every number that is either a multiple or a divisor and
    ...
    b. otherwise, we consider it an option
...
## 2.1 Completing the AI

If you start playing the game, you will notice that the computer always does the same steps (assuming you do the same steps).  The reason is that we did not initialize the random number generator.  We can do that as follows:
...
Now, we get at least varying behavior of the computer.
